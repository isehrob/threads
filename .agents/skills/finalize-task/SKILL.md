...
2. **Read the full task history** and all supporting files in `.tasks/<slug>/`.

3. **Propose a finalization plan.** Persist it as `.tasks/<slug>/finalize-plan.md`. The plan covers:
   ...
   - **AGENTS.md updates:** Any system-level convention changes.
   - **Adapter updates:** Any agent-specific changes (CLAUDE.md, etc.).

4. **Iterate with the user.** They review, adjust, push back. This may take multiple rounds. Context may get compacted — that's fine, the plan file persists. User can re-invoke this skill and point to the plan.

5. **Execute only after approval.** Apply all changes from the plan:
   - Write compressed `history.jsonl`
   - Write knowledge files to `~/agents/knowledge/`
   - Update skills, AGENTS.md, adapters as approved
   - Remove supporting files no longer needed
   - Remove finalize-plan.md

6. **Append a `completed` entry** to history (before compressing, so it's preserved in the compressed version).
...
- When updating skills or AGENTS.md, show exact diffs in the plan.
